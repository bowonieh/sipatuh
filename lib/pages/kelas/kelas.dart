...
class KelasData {
  ...
  factory KelasData.fromJson(Map<String, dynamic> json) {
    return KelasData(
      ...
      nama: json['nama'],
      tingkatId: int.tryParse(json['tingkat_id'].toString()) ?? 0,
      tingkatNama: json['tingkat_tingkat'],
      jurusanId: int.tryParse(json['jurusan_id'].toString()) ?? 0,
      jurusanNama: json['jurusan_nama'],
...
class _KelasState extends State<Kelas> {
  ...
  void fetchDataSiswa(String query) async {
    ...
    try {
      final dynamic apiUrl = Uri.parse(Constants.baseUrl + Constants.kelasapi+'$query');
      ...
      if (response.statusCode == 200 && response.body.isNotEmpty) {
        ...
        if (decodedData is List) {
          List<KelasData> newDataList =
              decodedData.map((item) => KelasData.fromJson(item)).toList();
          if (mounted) {
            setState(() {
